...
def main():
    ...
    print(f"{'Percentage':<6} {'Graph':<21} {'Size':<11} {'Subdirectory'}")
    print("-" * 55)

    # Sort directories by percentage size
    ...
    for directory, size in sorted_dirs:
        ...
        graph = percent_to_graph(percent, 20)  # Generate bar graph with fixed length of 20
        size_str = bytes_to_human_r(size) if args.human_readable else f"{size} bytes"
        print(f"{percent:.0f}% [{graph}] {size_str:<11} {directory}")
